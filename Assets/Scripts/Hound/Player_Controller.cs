...
public class Player_Controller : MonoBehaviour {
    ...
    private float moveSpeed, jumpForce, holdJump = 0, diveForce, bulletSpeed, spitCooldown, spitTimer, jumpCount, baseSpeed = 3f, boostSpeed = 5f, barkDistance, barkHeight, scaleX;

    private bool bat, shoe, rabbit, squirrel, isFull, jump2, walking, grounded, upsideDown, canSpit, timerOn;

	void Start () {
        ground = GameObject.FindGameObjectWithTag("Ground").GetComponent<PlatformEffector2D>();
    ...
    void movement()
    {
        ...
        {
            body.velocity = new Vector2(moveVector.x * moveSpeed, body.velocity.y);
            walking = true;
        }
        else
        {
        ...
        if(bat)
        {
            if(!grounded)
            {
                if (!upsideDown)
                {
                    ...
                    if (Input.GetKeyDown(KeyCode.W) && !jump2)
                    {
                        body.velocity = new Vector2(body.velocity.x, 0);
                        body.AddForce(new Vector2(0, jumpForce), ForceMode2D.Impulse);
                        jump2 = true;
                ...
                else if (upsideDown)
                {
                    ...
                    if (Input.GetKeyDown(KeyCode.S) && !jump2)
                    {
                        body.velocity = new Vector2(body.velocity.x, 0);
                        body.AddForce(new Vector2(0, -jumpForce), ForceMode2D.Impulse);
                        jump2 = true;
    ...
    void spit()
    {
        if (stomach1 != 0)
        {
            if (Input.GetKey(KeyCode.R) && canSpit)
            {
    ...
    void detectGround()
    {
        ...
        hit = Physics2D.Linecast(transform.position, new Vector2(transform.position.x, feet.transform.position.y), 1 << LayerMask.NameToLayer("PlayerGround"));
        if(hit.collider != null)
        {
            grounded = true;
        }
